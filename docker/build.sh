...
log_info "dockerfile" "DOCKERFILE_FOLDER : $DOCKERFILE_FOLDER; DOCKERFILE_NAME : $DOCKERFILE_NAME"

if [ -z "$build_dir" ]; then
  log_info "build_dir" "build_dir is empty then use DOCKERFILE_FOLDER"
...
function build_push() {
  ...
  if [ "$multi_platform" == "true" ]; then
    ...
    log_info "docker_build" "docker build -f $path_to_dockerfile -t $image_name:$image_tag $build_dir"
