...
function clean_data_log() {
  if [ -d $log_dir ]; then
    read -p "是否清空日志目录 $log_dir? (y/n): " cleanLog
    if [ "$cleanLog" == "y" ]; then
      rm -rf $log_dir/*
  ...
  if [ -d $data_dir ]; then
    read -p "是否清空数据目录 $data_dir? (y/n): " cleanData
    if [ "$cleanData" == "y" ]; then
      rm -rf $data_dir/*
...
read -p "是否清空数据和日志目录? (y/n): " cleanAll
if [ "$cleanAll" == "y" ]; then
  clean_data_log
else
  echo "未清空数据和日志目录"
